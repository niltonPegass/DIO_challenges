...
necessario_cadastro = "[!] É NECESSÁRIO CADASTRAR O USUÁRIO"
...
def cadastrar_conta(cpf):
    ...
    if cadastro_conta.get(cpf):
        ...
        if opcao == "N":
            ...
            cadastro_conta[cpf] = {"agencia": agencia, "conta": conta, "usuario": cpf}
        ...
        if cadastro_usuario.get(cpf):
            ...
            conta = input("INFORME A CONTA: ")
            cadastro_conta[cpf] = {"agencia": agencia, "conta": conta, "usuario": cpf}
            print(f">> USUARIO {cpf} CADASTRADO:\nAGENCIA: {agencia} // CONTA: {conta}")

        else:
            print(f"{necessario_cadastro}")
def listar_contas():
